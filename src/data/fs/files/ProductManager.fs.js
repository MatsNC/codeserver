...
class ProductManager {
  ...
  async update(id, product) {
    try {
      ...
      let prodToUpdate = all.find((each) => each.id === id);
      if (prodToUpdate) {
        for (let prop in product) {
          prodToUpdate[prop] = product[prop];
        }
        ...
        throw error;    //hace saltar el catch de abajo
      }
    } catch (error) {
      throw error;      //hace sa;tar el catch de la ruta
    }
  }
}
